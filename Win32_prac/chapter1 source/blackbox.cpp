...
#include <windowsx.h> 
#include <mmsystem.h>		//声音有关

#include <iostream.h>        // include important C/C++ stuff
#include <conio.h>
//　conio是Console Input/Output（控制台输入输出）的简写，其中定义了通过
//控制台进行数据输入和数据输出的函数，主要是一些用户通过按键盘产生的对
//应操作，比如getch()函数等等。
//　　大部分DOS，Windows 3.x，Phar Lap，DOSX，OS/2 or Win32平台上的
//C编译器提供此文件，UNIX 和Linux平台的c编译器通常不包含此头文件。
...
宏例如EXIT_FAILURE、EXIT_SUCCESS、RAND_MAX和MB_CUR_MAX等等；
 ...
 atol()、rand()、srand()、exit()等等。 具体的内容你自己可以打开编译器的
 *///include目录里面的stdlib.h头文件看看。
#include <malloc.h>
#include <memory.h>
#include <string.h>
#include <stdarg.h>	
...
#include <fcntl.h>		
//
#include <ddraw.h>           // 包含 directX ，PS:在 工程(project)下设置(setting)里的连接(link) 中有一个 挺多lib的，隔开写入ddraw.lib。
#include "blackbox.h"        // game library includes 包含游戏库
